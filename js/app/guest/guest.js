...
export const guest = (() => {
     ...
     */
    const modal = (img) => {
        const modalClickBtn = document.getElementById('button-modal-click');
        const modalDownloadBtn = document.getElementById('button-modal-download');
        const showModalImage = document.getElementById('show-modal-image');
        
        if (modalClickBtn && modalDownloadBtn && showModalImage) {
            modalClickBtn.setAttribute('href', img.src);
            modalDownloadBtn.setAttribute('data-src', img.src);

            showModalImage.src = img.src;
            showModalImage.width = img.width;
            showModalImage.height = img.height;
    ...
    const modalImageClick = () => {
        const showModalImage = document.getElementById('show-modal-image');
        if (showModalImage) {
            showModalImage.addEventListener('click', (e) => {
    ...
    const buildGoogleCalendar = () => {
        ...
        const data = new URLSearchParams({
            ...
            details: 'With all due respect, we invite you to attend our wedding ceremony. Thank you for your attention and prayers, which bring us great happiness and honor.',
    ...
    const pageLoaded = () => {
        ...
        // Only initialize comments if comment section exists
        ...
        if (token && token.length > 0) {
            ...
            session.guest(params.get('k') ?? token).then(({ data }) => {
                ...
                // Only initialize comments if the comment section exists
                const commentSection = document.getElementById('comment');
                if (commentSection) {
                    comment.show()
